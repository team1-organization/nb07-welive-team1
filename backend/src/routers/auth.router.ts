...
//router.patch('/admins/:adminId/status', withAsync(authController.updateAdminStatus));
// [슈퍼관리자] 관리자 가입 상태 일괄 변경
//router.patch('/admins/status', withAsync(authController.updateManyAdminStatus));
// [관리자] 주민 가입 상태 변경(단건)
//router.patch(`/residents/:residentId/status`, withAsync(authController.updateResidentStatus));
// [관리자] 주민 가입 상태 일괄 변경
//router.patch(`/residents/status`, withAsync(authController.updateManyResidentStatus));
// [슈퍼관리자] 관리자 정보(아파트 정보) 수정
//router.patch(`/admins/:adminId`, withAsync(authController.updateAdminInfo));
// [슈퍼관리자] 관리자 정보(아파트 정보 포함) 삭제
//router.delete(`/admins/:adminId`, withAsync(authController.deleteAdmin));
// [슈퍼관리자/관리자] 거절 계정 관리(최고 관리자는 관리자 계정을, 관리자는 사용자 계정을 일괄 정리)
//router.post(`/cleanup`, withAsync(authController.cleanupRejectedUsers));
