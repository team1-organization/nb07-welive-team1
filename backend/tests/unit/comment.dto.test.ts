import { CreateCommentDto, DeleteCommentParamSchema, UpdateCommentDto } from '../../src/dtos/comment.dto';
...
describe('Comment DTO 유닛 테스트', () => {
  ...
  describe('UpdateCommentDto (댓글 수정)', () => {
    it('올바른 content가 들어오면 통과해야 한다', () => {
      const validData = {
        content: '수정된 댓글입니다.',
      };

      const result = UpdateCommentDto.safeParse(validData);
      expect(result.success).toBe(true);
    });
    ...
    it('내용(content)이 비어있으면 실패해야 한다', () => {
      ...
      const result = UpdateCommentDto.safeParse(invalidData);
      expect(result.success).toBe(false);
    });

    it('boardType, boardId 없이도 통과해야 한다', () => {
      const validData = {
        content: '댓글 수정 테스트',
      };
